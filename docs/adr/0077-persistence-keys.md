...
**Surrogate UUIDs:** UUIDv7 for internal non-bearer rows (Refresh Token Family, Audit Event, and similar). UUIDv4 for values exported as capability-adjacent identifiers — User PK / Access Token `sub`, and Access Token `jti`. Soft-delete uses `deactivated_at timestamptz NULL` (NULL = active).

**Composite natural PKs:** Redirect URI registrations are `(client_id, redirect_uri)`. Consent Grants are `(user_id, client_id, scope)`; revoke sets `deactivated_at`; re-grant clears it (reactivate) rather than inserting a new row; both mutations are Audit Events.
